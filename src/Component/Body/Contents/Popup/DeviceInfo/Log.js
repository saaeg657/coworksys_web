    ...
    render() {
        const mapToComponent = (data) => {
            ...
            return data.map((item, i) => {
                return (
                    <div key={i} className="div-deviceinfo-network" style={{ backgroundColor: i % 2 == 0 ? "#f7f7f7" : "white", paddingLeft: 20 }}>
                        <div style={{ width: 180 }}>{item.User}</div>
                        <div style={{ width: 180 }}>{item.Time}</div>
                        <div style={{ width: 200 }}>{item.Action}</div>
                    </div>
                );
            })
        };
            ...
            <div className="div-deviceinfo-popup">
                <div className="div-block-58" style={{ marginLeft: 30 }}>
                    <div className="div-block-59" style={{ marginTop: 20, width: 725, justifyContent: "space-between" }}>
                        <div>{"Total : " + (this.props.history != undefined ? this.props.history.length : 0)}</div>
                    ...
                    <div className="div-block-61">
                        {mapToComponent(this.props.history)}
                    </div>
                </div>
            </div>
        );
